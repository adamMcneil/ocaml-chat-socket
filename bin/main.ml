...
let rec send_messages client_socket =
  let message = read_line () in
  match message with
  | "quit" -> ()
  | _ ->
      let bytes = Bytes.of_string message in
      (* let _ = Unix.write client_socket bytes 0 (Bytes.length bytes) in *)
      let _ =
        Unix.send client_socket bytes 0 (Bytes.length bytes) [ Unix.MSG_OOB ]
      in
      let _ = fflush client_socket in
      send_messages client_socket

let rec recv_messages socket =
